        ...
        if (msg.action === "activateTerrain") {
                ...
                if (myLocation) {
                    ...
                    try {
                        if (reearth && reearth.camera && typeof reearth.camera.flyTo === 'function')
                            reearth.camera.flyTo({ lat: myLocation.lat, lng: myLocation.lng, height: 1000, heading: 0, pitch: -1.57, roll: 0 }, { duration: flyDuration });
                    }
                    catch (e) { }
                    // Wait for flyTo to complete (duration + small buffer) before sampling terrain height
                    try {
                        await new Promise(res => setTimeout(res, Math.round(flyDuration * 1000) + 300));
                    }
                    catch (e) { }
            ...
            try {
                ...
                try {
                    ...
                    try {
                        sendError('[flyToManual] addMarker error:', e);
                    }
                    catch (_) { }
                }
            }
            ...
            try {
                ...
                if (typeof idx === 'number' && _cameraPresets[idx]) {
                    ...
                    try {
                        sendLog('[flyToCamera] flying to:', cam.title, cam.lat, cam.lng);
                    }
                    catch (e) { }
                }
            }
