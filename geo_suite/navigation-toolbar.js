"use strict";
// Navigation Toolbar Plugin for Re:Earth — simplified: only click-to-north and Sync
// @ts-nocheck
...
const html = `
  <style>
    ...
    /* needle wrapper sits exactly at compass center; rotate the wrapper so rotation pivot is always compass center */
    ...
    /* place N between center and top rim (midway) */
  ...
  <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:6px;">
    <div style="position:relative;display:flex;align-items:center;justify-content:center">
      <div class="compass" id="compass">
        <!-- wrapper positioned at compass center; rotate this wrapper to pivot around center -->
        <div id="needleWrap" class="needle-wrap">
          <svg id="needle" class="needle-svg" viewBox="0 0 24 40" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <!-- compact arrow: head then straight shaft; shaft extended 1.5x -->
            <polygon points="12,2 20,18 12,14 4,18" fill="#ff4d4d" />
            <!-- shaft extended to protrude through center -->
...
function postToUI(msg) {
  ...
  try {
    if (typeof parent !== 'undefined' && parent && typeof parent.postMessage === 'function') parent.postMessage(msg, '*');
  } catch (e) {}
}

// Message Handler (messages from UI -> host)
const onMessage = (msg) => {
  if (!msg || !msg.action) return;
  // setHeading: rotate to heading (preserve other camera properties if available)
  if (msg.action === 'setHeading') {
    try {
      var target = { heading: msg.payload && typeof msg.payload.heading === 'number' ? msg.payload.heading : 0 };
      var cur = (typeof reearth !== 'undefined' && reearth && reearth.camera && reearth.camera.position) ? reearth.camera.position : null;
      if (cur) {
  ...
  // requestCamera: reply with current heading
  if (msg.action === 'requestCamera') {
    try{
      var cur2 = (typeof reearth !== 'undefined' && reearth && reearth.camera && reearth.camera.position) ? reearth.camera.position : null;
      var h = cur2 && typeof cur2.heading === 'number' ? cur2.heading : undefined;
      postToUI({ type: 'cameraUpdate', payload: { heading: h } });
    }catch(e){}
  }
};

// Register Event Listeners (listen for messages from UI)
try {
