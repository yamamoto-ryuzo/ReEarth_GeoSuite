...
function getUI() {
    try {
        ...
        const parseGroupPath = (groupStr) => {
            ...
            const res = [];
            if (!groupStr || typeof groupStr !== 'string')
                return res;
            let i = 0;
            let cur = '';
            while (i < groupStr.length) {
                ...
                if (ch === '/' || ch === "\\") {
                    ...
                    let j = i;
                    while (j < groupStr.length && groupStr[j] === ch)
                        j++;
                    const sepCount = j - i;
                    if (cur !== '') {
            ...
            if (cur !== '')
                res.push({ seg: cur, sepChar: null, sepCount: 0, exclusiveAfter: false, expandAfter: false });
            return res;
        };
        const buildTree = (arr) => {
            ...
            arr.forEach(layer => {
                try {
                    ...
                    let node = root;
                    if (layer && layer.id)
                        root.allLayerIds.push(layer.id);
                    for (let k = 0; k < parsed.length; k++) {
                        ...
                        if (layer && layer.id)
                            node.allLayerIds.push(layer.id);
                    }
...
try {
    if (typeof tryInitFromProperty === 'function')
        tryInitFromProperty();
}
catch (e) { }
...
try {
    ...
    if (textToProcess && textToProcess.trim()) {
        try {
            ...
            try {
                sendError('[init] processInspectorText failed', e);
            }
            catch (_) { }
        }
    }
...
try {
    ...
    if (reearth.ui && typeof reearth.ui.postMessage === 'function') {
        ...
        try {
            sendLog('[init] sending depthTest state to UI', { enabled: depthTest });
        }
        catch (e) { }
...
function postToUI(msg) {
    try {
        if (reearth && reearth.ui && typeof reearth.ui.postMessage === 'function') {
            try {
                sendLog('[postToUI] using reearth.ui.postMessage', msg && msg.action ? msg.action : msg);
            }
            catch (e) { }
    ...
    try {
        if (typeof window !== 'undefined' && window.parent && typeof window.parent.postMessage === 'function') {
            try {
                sendLog('[postToUI] falling back to window.parent.postMessage', msg && msg.action ? msg.action : msg);
            }
            catch (e) { }
            window.parent.postMessage(msg, '*');
            return;
    ...
    try {
        if (typeof parent !== 'undefined' && parent && typeof parent.postMessage === 'function') {
            try {
                sendLog('[postToUI] falling back to parent.postMessage', msg && msg.action ? msg.action : msg);
            }
            catch (e) { }
    ...
    try {
        if (state.lat != null && state.lng != null) {
            try {
                ...
                try {
                    sendError('[applyPermalinkFromObject] camera flyTo failed', e);
                }
                catch (err) { }
            }
        }
        const applyLayersWithRetry = (layersStr, attempt = 1) => {
            try {
                ...
                if (!layersApiAvailable || (reearth.layers.layers && reearth.layers.layers.length === 0)) {
                    if (attempt <= maxAttempts) {
                        try {
                            sendLog('[applyPermalinkFromObject] layers not ready, retry', attempt);
                        }
                        catch (e) { }
                        setTimeout(() => applyLayersWithRetry(layersStr, attempt + 1), delayMs);
                        return;
                        ...
                        try {
                            sendError('[applyPermalinkFromObject] layers unavailable after retries');
                        }
                        catch (e) { }
                        try {
                            reearth.ui.postMessage({ action: 'permalinkApplied', success: false, reason: 'layers_unavailable' });
                        }
                        catch (e) { }
                        return;
                    }
                ...
                try {
                    sendLog('[applyPermalinkFromObject] applied layers', { requested: ids.length, found: found, changed: applied });
                }
                catch (e) { }
                try {
                    reearth.ui.postMessage({ action: 'permalinkApplied', success: true, requested: ids.length, found: found, changed: applied });
                }
                catch (e) { }
            }
            catch (e) {
                if (attempt <= 8)
                    setTimeout(() => applyLayersWithRetry(layersStr, attempt + 1), 800);
                else
                    try {
                        sendError('[applyPermalinkFromObject] unexpected error applying layers', e);
                    }
                    catch (err) { }
            }
        };
        if (state.layers)
            applyLayersWithRetry(state.layers);
    }
    catch (e) {
        try {
            sendError('[applyPermalinkFromObject] error:', e);
        }
        catch (err) { }
    }
}
...
function safeShowUI(context) {
    try {
        try {
            sendLog('[safeShowUI] context:', context);
        }
        catch (e) { }
        // capture stack to help identify call sites at runtime
        try {
            sendLog('[safeShowUI] stack:', (new Error()).stack);
        }
        catch (e) { }
        if (reearth && reearth.ui && typeof reearth.ui.show === 'function') {
            try {
                ...
                try {
                    sendError('[safeShowUI] show failed', e);
                }
                catch (_) { }
            }
        }
        ...
        try {
            sendError('[safeShowUI] unexpected', e);
        }
        catch (_) { }
    }
}
...
function urlsEqual(a, b) {
    ...
    try {
        if (encodeNonAscii(a || '') === encodeNonAscii(b || ''))
            return true;
    }
    catch (e) { }
    return false;
...
function setLayerVisibility(layerId, visible, renderUI = true) {
    ...
    try {
        ...
        try {
            sendError('[setLayerVisibility] unexpected error', e);
        }
        catch (_) { }
    }
    try {
...
async function addTargetMarker(lat, lng) {
    try {
        ...
        try {
            ...
            try {
                sendError('[addTargetMarker] reearth.layers.add threw:', e);
            }
            catch (_) { }
        }
        if (layerId) {
        ...
        try {
            sendError('[addTargetMarker] error:', e);
        }
        catch (_) { }
        return null;
    }
...
async function getCurrentLocation() {
    ...
    try {
        ...
        if (reearth && reearth.viewer && reearth.viewer.tools && typeof reearth.viewer.tools.getCurrentLocationAsync === 'function') {
            try {
                sendLog('[getCurrentLocation] using viewer.tools.getCurrentLocationAsync');
            }
            catch (e) { }
        ...
        try {
            sendLog('[getCurrentLocation] trying fallback location sources');
        }
        catch (e) { }
        let cur = null;
        try {
            cur = (reearth.camera && typeof reearth.camera.position === 'object' && reearth.camera.position) ? reearth.camera.position : null;
        }
        catch (e) { }
        if (!cur)
            try {
                cur = (reearth.camera && typeof reearth.camera.getCamera === 'function') ? reearth.camera.getCamera() : null;
            }
            catch (e) { }
        if (!cur)
            try {
                cur = (reearth.viewer && typeof reearth.viewer.getCamera === 'function') ? reearth.viewer.getCamera() : null;
            }
            catch (e) { }
        if (!cur)
            try {
                cur = (reearth.view && reearth.view.camera) ? reearth.view.camera : null;
            }
            catch (e) { }
        if (!cur)
            try {
                cur = reearth.camera || null;
            }
            catch (e) { }
        if (cur && (cur.lat != null || cur.latitude != null)) {
            const lat = (_b = (_a = cur.lat) !== null && _a !== void 0 ? _a : cur.latitude) !== null && _b !== void 0 ? _b : null;
            const lng = (_e = (_d = (_c = cur.lng) !== null && _c !== void 0 ? _c : cur.longitude) !== null && _d !== void 0 ? _d : cur.lon) !== null && _e !== void 0 ? _e : null;
        ...
        try {
            sendError('[getCurrentLocation] error:', e);
        }
        catch (_) { }
        return null;
    }
...
async function flyToAndNotify(lat, lng, opts) {
    ...
    try {
        ...
        try {
            ...
            try {
                sendError('[flyToAndNotify] flyTo threw', e);
            }
            catch (_) { }
        }
        try {
            ...
            try {
                sendLog('[flyToAndNotify] waiting', waitMs, 'ms before addTargetMarker');
            }
            catch (e) { }
            if (typeof setTimeout === 'function') {
                await new Promise(res => setTimeout(res, waitMs));
                try {
                    sendLog('[flyToAndNotify] wait complete');
                }
                catch (e) { }
            }
            else {
                try {
                    sendError('[flyToAndNotify] setTimeout not available in this runtime; skipping wait');
                }
                catch (e) { }
            }
        }
            ...
            try {
                sendError('[flyToAndNotify] wait threw', e);
            }
            catch (_) { }
        }
        try {
            sendLog('[flyToAndNotify] proceeding to addTargetMarker (post-wait)');
        }
        catch (e) { }
        let layerId = null;
        if (addMarkerFlag && typeof addTargetMarker === 'function' && !isNaN(lat) && !isNaN(lng)) {
            try {
                ...
                try {
                    sendError('[flyToAndNotify] addTargetMarker threw', e);
                }
                catch (_) { }
                layerId = null;
            }
...
try {
    ...
    try {
        sendError("Failed to register layer select event", e);
    }
    catch (_) { }
}
        ...
        if (msg.action === "activateTerrain") {
            ...
            try {
                ...
                if (cur && reearth.ui && typeof reearth.ui.postMessage === 'function') {
                    ...
                    const lat = (_b = (_a = cur.lat) !== null && _a !== void 0 ? _a : cur.latitude) !== null && _b !== void 0 ? _b : null;
                    const lng = (_e = (_d = (_c = cur.lng) !== null && _c !== void 0 ? _c : cur.longitude) !== null && _d !== void 0 ? _d : cur.lon) !== null && _e !== void 0 ? _e : null;
                    const h = (_h = (_g = (_f = cur.height) !== null && _f !== void 0 ? _f : cur.altitude) !== null && _g !== void 0 ? _g : cur.alt) !== null && _h !== void 0 ? _h : null;
                    const heading = (_k = (_j = cur.heading) !== null && _j !== void 0 ? _j : cur.yaw) !== null && _k !== void 0 ? _k : null;
                    const pitch = (_m = (_l = cur.pitch) !== null && _l !== void 0 ? _l : cur.tilt) !== null && _m !== void 0 ? _m : null;
                ...
                try {
                    sendError('[requestCamera] error:', e);
                }
                catch (err) { }
            }
        }
            ...
            try {
                ...
                if (typeof idx === 'number' && _cameraPresets[idx]) {
                    ...
                    if (cur) {
                        const rad2deg = (r) => typeof r === 'number' ? Math.round(r * 180 / Math.PI * 100) / 100 : 0;
                        const lat = (_p = (_o = cur.lat) !== null && _o !== void 0 ? _o : cur.latitude) !== null && _p !== void 0 ? _p : 0;
                        const lng = (_s = (_r = (_q = cur.lng) !== null && _q !== void 0 ? _q : cur.longitude) !== null && _r !== void 0 ? _r : cur.lon) !== null && _s !== void 0 ? _s : 0;
                        const h = (_v = (_u = (_t = cur.height) !== null && _t !== void 0 ? _t : cur.altitude) !== null && _u !== void 0 ? _u : cur.alt) !== null && _v !== void 0 ? _v : 1000;
                        const heading = (_x = (_w = cur.heading) !== null && _w !== void 0 ? _w : cur.yaw) !== null && _x !== void 0 ? _x : 0;
                        const pitch = (_z = (_y = cur.pitch) !== null && _y !== void 0 ? _y : cur.tilt) !== null && _z !== void 0 ? _z : 0;
                        // プリセット更新
                        ...
                        // UI 再レンダリング
                        try {
                            safeShowUI('updateCamPreset');
                        }
                        catch (e) { }
                        try {
                            sendLog('[updateCamPreset] updated preset', idx, _cameraPresets[idx].title);
                        }
                        catch (e) { }
                    }
                }
                ...
                try {
                    sendError('[updateCamPreset] error:', e);
                }
                catch (err) { }
            }
        }
            ...
            if (msg.layerId) {
                try {
                    try {
                        sendLog('[removeLayer] requested for', msg.layerId);
                    }
                    catch (e) { }
                    removeTargetMarker(msg.layerId);
                }
                    ...
                    try {
                        sendError('[removeLayer] failed to delete layer:', e);
                    }
                    catch (err) { }
                }
            }
            ...
            try {
                if (msg.url && reearth && reearth.viewer && typeof reearth.viewer.open === "function") {
                    ...
                    // fallback fallback
                    try {
                        sendLog('[openUrl] reearth.viewer.open is not available, falling back');
                    }
                    catch (e) { }
                }
            }
            ...
            try {
                ...
                for (let i = 0; i < existing.length; i++) {
                    const l = existing[i];
                    try {
                        if (l && l.data && l.data.isBasemap && l.data.url && urlsEqual(l.data.url, url)) {
                            found = l;
                            break;
                        }
                    }
                ...
                try {
                    _lastAddedBasemapUrl = encodeNonAscii(url);
                }
                catch (e) { }
                try {
            ...
            try {
                ...
                if (typeof idx === 'number' && _cameraPresets[idx]) {
                    ...
                    try {
                        ...
                        if (cur) {
                            const h = (_2 = (_1 = (_0 = cur.height) !== null && _0 !== void 0 ? _0 : cur.altitude) !== null && _1 !== void 0 ? _1 : cur.alt) !== null && _2 !== void 0 ? _2 : null;
                            const hd = (_5 = (_4 = (_3 = cur.heading) !== null && _3 !== void 0 ? _3 : cur.yaw) !== null && _4 !== void 0 ? _4 : cur.h) !== null && _5 !== void 0 ? _5 : null;
                            const p = (_8 = (_7 = (_6 = cur.pitch) !== null && _6 !== void 0 ? _6 : cur.tilt) !== null && _7 !== void 0 ? _7 : cur.p) !== null && _8 !== void 0 ? _8 : null;
                            const r = (_10 = (_9 = cur.roll) !== null && _9 !== void 0 ? _9 : cur.r) !== null && _10 !== void 0 ? _10 : null;
                            if (typeof h === 'number')
                                curHeight = h;
                            if (typeof hd === 'number')
                                curHeading = hd;
                    ...
                    (async () => { try {
                        ...
                        try {
                            sendError('[flyToAndNotify] flyToCamera failed', e);
                        }
                        catch (_) { }
                    } })();
                }
                ...
                try {
                    sendError('[flyToAndNotify] flyToCamera outer error:', e);
                }
                catch (err) { }
            }
        }
            ...
            (async () => {
                try {
                    ...
                    try {
                        sendError('[flyToAndNotify] flyToManual failed', e);
                    }
                    catch (_) { }
                }
            })();
            ...
            try {
                ...
                if (cur) {
                    ...
                    const lat = (_12 = (_11 = cur.lat) !== null && _11 !== void 0 ? _11 : cur.latitude) !== null && _12 !== void 0 ? _12 : null;
                    const lng = (_15 = (_14 = (_13 = cur.lng) !== null && _13 !== void 0 ? _13 : cur.longitude) !== null && _14 !== void 0 ? _14 : cur.lon) !== null && _15 !== void 0 ? _15 : null;
                    const h = (_18 = (_17 = (_16 = cur.height) !== null && _16 !== void 0 ? _16 : cur.altitude) !== null && _17 !== void 0 ? _17 : cur.alt) !== null && _18 !== void 0 ? _18 : null;
                    const heading = (_20 = (_19 = cur.heading) !== null && _19 !== void 0 ? _19 : cur.yaw) !== null && _20 !== void 0 ? _20 : null;
                    const pitch = (_22 = (_21 = cur.pitch) !== null && _21 !== void 0 ? _21 : cur.tilt) !== null && _22 !== void 0 ? _22 : null;
                    if (typeof lat === 'number')
                        payload.lat = Math.round(lat * 1000000) / 1000000;
                    if (typeof lng === 'number')
                        payload.lng = Math.round(lng * 1000000) / 1000000;
                    if (typeof h === 'number')
                        payload.height = Math.round(h * 10) / 10;
                    if (typeof heading === 'number')
                        payload.heading = rad2deg(heading);
                    if (typeof pitch === 'number')
                        payload.pitch = rad2deg(pitch);
                }
                if (reearth.ui) {
                ...
                try {
                    sendError('[generatePermalink] error:', e);
                }
                catch (err) { }
            }
        }
            ...
            try {
                ...
                try {
                    sendError('[applyPermalinkState] error:', e);
                }
                catch (err) { }
            }
        }
            ...
            try {
                let query = null;
                try {
                    query = (reearth.viewer && reearth.viewer.viewport && reearth.viewer.viewport.query) ? reearth.viewer.viewport.query : null;
                }
                catch (e) { }
                if (!query)
                    try {
                ...
                if (Object.keys(query).length > 0) {
                    ...
                    if (query.height != null)
                        state.height = parseFloat(query.height);
                    if (query.heading != null)
                        state.heading = parseFloat(query.heading);
                    if (query.pitch != null)
                        state.pitch = parseFloat(query.pitch);
                    if (query.layers != null)
                        state.layers = query.layers;
                    if (typeof state.lat === 'number' && !isNaN(state.lat) && typeof state.lng === 'number' && !isNaN(state.lng)) {
                        ...
                        try {
                            sendError('[flyToViewportUrlParams] invalid lat/lng in query:', query);
                        }
                        catch (e) { }
                    }
                }
                    ...
                    try {
                        sendError('[flyToViewportUrlParams] viewport query unavailable');
                    }
                    catch (e) { }
                }
            }
                ...
                try {
                    sendError('[flyToViewportUrlParams] error:', e);
                }
                catch (err) { }
            }
        }
    ...
    switch (msg.type) {
        case "flyTo":
            try {
                ...
                try {
                    ...
                    try {
                        sendError('[flyToAndNotify] flyTo by layer failed', e);
                    }
                    catch (_) { }
                }
            }
        ...
        case "hide":
            try {
                ...
                try {
                    sendError('[hide] error setting visibility', msg.layerId, e);
                }
                catch (_) { }
            }
            break;
        case "show":
            try {
                ...
                try {
                    sendError('[show] error setting visibility', msg.layerId, e);
                }
                catch (_) { }
            }
            break;
...
function tryInitFromProperty() {
    ...
    try {
        ...
        try {
            sendLog('[init] raw property object:', prop);
        }
        catch (e) { }
        try {
            // send property to UI for debugging (UI console will show it)
            try {
                postToUI({ action: 'debugInspectorProperty', prop: prop });
            }
            catch (e) { }
        }
        catch (e) { }
...
function addXyzLayer(url, title, layerType, isBase = false, visible = true) {
    ...
    try {
        ...
        try {
            ...
            try {
                sendError('[addXyzLayer] unexpected error during UI update:', e);
            }
            catch (err) { }
        }
        return newId;
...
try {
    ...
    if (textToProcess && textToProcess.trim()) {
        try {
            sendLog('[init] processing inspector text at startup, length:', textToProcess.length, 'isDefault:', textToProcess === _defaultInspectorText);
        }
        catch (e) { }
        processInspectorText(textToProcess);
    }
...
function rebuildInspectorText() {
    try {
        ...
        _cameraPresets.forEach(function (cam) {
            ...
            if (cam.height !== null && cam.height !== undefined)
                camLine += '|h=' + cam.height;
            if (cam.heading !== null && cam.heading !== undefined)
                camLine += '|d=' + rad2deg(cam.heading);
            if (cam.pitch !== null && cam.pitch !== undefined)
                camLine += '|p=' + rad2deg(cam.pitch);
            lines.push(camLine);
        });
        ...
        try {
            ...
            try {
                sendLog('[rebuildInspectorText] setPropertyValue not available, cache only');
            }
            catch (_) { }
        }
        try {
            sendLog('[rebuildInspectorText] rebuilt:', newText.substring(0, 200));
        }
        catch (e) { }
    }
    catch (e) {
        try {
            sendError('[rebuildInspectorText] error:', e);
        }
        catch (_) { }
    }
}
...
function processInspectorText(text) {
    ...
    lines.forEach(line => {
        ...
        if (lowerLine.startsWith('legend:')) {
            ...
            if (content) {
                ...
                if (url)
                    legends.push({ group: group, url: encodeNonAscii(url) });
            }
        ...
        if (lowerLine.startsWith('background:') || lowerLine.startsWith('bg:')) {
            ...
            if (col) {
                try {
                    sendLog('[processInspectorText] found BACKGROUND color:', col);
                }
                catch (e) { }
                if (col !== _lastInspectorBackground) {
                    ...
                    try {
                        ...
                        try {
                            sendError('[processInspectorText] failed to apply background color', e);
                        }
                        catch (_) { }
                    }
                }
        ...
        if (lowerLine.startsWith('info:')) {
            ...
            if (url) {
                ...
                try {
                    sendLog('[processInspectorText] found INFO url:', infoUrlFound);
                }
                catch (e) { }
            }
            nonCamLines.push(line);
        ...
        if (/^yahooappid\s*:/i.test(lowerLine)) {
            try {
                ...
                if (val) {
                    ...
                    try {
                        sendLog('[processInspectorText] found inspector yahooAppId');
                    }
                    catch (e) { }
                }
            }
        ...
        if (lowerLine.startsWith('layer:') || lowerLine.startsWith('preset:')) {
            ...
            if (content) {
                ...
                if (parts.length > 1) {
                    ...
                    if (v === 'off' || v === 'false' || v === 'hide' || v === 'invisible')
                        visible = false;
                }
                if (name) {
        ...
        if (lowerLine.startsWith('cam:')) {
            ...
            if (parts.length >= 3) {
                ...
                if (!isNaN(lat) && !isNaN(lng) && lat >= -90 && lat <= 90 && lng >= -180 && lng <= 180) {
                    ...
                    camsFound.push(cam);
                    try {
                        sendLog('[processInspectorText] found CAM:', cam.title, cam.lat, cam.lng, 'h:', cam.height, 'd:', cam.heading, 'p:', cam.pitch);
                    }
                    catch (e) { }
                }
            }
        ...
        if (lowerLine.startsWith('3dtiles:') || lowerLine.startsWith('3d-tiles:')) {
            ...
            if (url)
                tiles.push({ url, title, type: '3dtiles', visible });
            nonCamLines.push(line);
            return;
        }
        // GeoJSON
        if (lowerLine.startsWith('geojson:')) {
            ...
            if (url)
                tiles.push({ url, title, type: 'geojson', visible });
            nonCamLines.push(line);
            return;
        }
        ...
        if (url) {
            ...
            if (isBase)
                _parsedBaseTiles.push({ url, title, attribution });
        }
        nonCamLines.push(line);
    });
    ...
    try {
        _inspectorLegendItems = (legends && legends.length) ? legends : [];
    }
    catch (e) { }
    if (tiles.length > 0) {
        try {
        ...
        try {
            if ((!_lastAddedBasemapUrl || _lastAddedBasemapUrl === '') && _parsedBaseTiles && _parsedBaseTiles.length) {
                try {
                    _lastAddedBasemapUrl = encodeNonAscii(_parsedBaseTiles[0].url);
                }
                catch (e) { }
            }
        }
        ...
        try {
            if (_lastAddedBasemapUrl) {
                ...
                try {
                    safeShowUI('processInspectorText: initial basemap apply');
                }
                catch (e) { }
            }
        }
    ...
    try {
        applySystemLayerSettings();
    }
    catch (e) { }
    try {
        safeShowUI('processInspectorText: final render');
    }
    catch (e) { }
    ...
    try {
        if (typeof setTimeout === 'function') {
            setTimeout(function () {
                try {
                    sendLog('[processInspectorText] sending legends count:', legends ? legends.length : 0);
                }
                catch (e) { }
                try {
                    if (legends && legends.length > 0)
                        postToUI({ action: 'updateLegends', items: legends });
                }
                catch (e) {
                    try {
                        sendError('[processInspectorText] updateLegends post failed', e);
                    }
                    catch (_) { }
                }
                try {
                    if (infoUrlFound && infoUrlFound !== _lastInfoUrl) {
                        try {
                            sendLog('[processInspectorText] applying INFO url (deferred):', infoUrlFound);
                        }
                        catch (e) { }
                    ...
                    try {
                        sendError('[processInspectorText] loadInfoUrl post failed', e);
                    }
                    catch (_) { }
                }
            }, 50);
            ...
            try {
                sendLog('[processInspectorText] sending legends count (no timeout):', legends ? legends.length : 0);
            }
            catch (e) { }
            try {
                if (legends && legends.length > 0)
            ...
            try {
                if (infoUrlFound && infoUrlFound !== _lastInfoUrl) {
                    try {
                        sendLog('[processInspectorText] applying INFO url (immediate):', infoUrlFound);
                    }
                    catch (e) { }
                    _lastInfoUrl = infoUrlFound;
                    postToUI({ action: 'loadInfoUrl', url: infoUrlFound });
        ...
        try {
            sendError('[processInspectorText] deferred post error', e);
        }
        catch (_) { }
    }
}
function restoreUserLayers(userRequests, force = false) {
    if (!reearth.layers || !reearth.layers.layers)
        return;
    try {
...
function applySystemLayerSettings() {
    if (!_systemLayerSettings || !_systemLayerSettings.length)
        return;
    ...
    if (changed) {
        try {
            safeShowUI('applySystemLayerSettings');
        }
        catch (e) { }
    }
}
    ...
    const poll = function () {
        try {
            ...
            if (text && typeof text === 'string' && text !== _lastInspectorLayersJson) {
                ...
                try {
                    sendLog('[poll] inspector text changed, length:', text.length);
                }
                catch (e) { }
                processInspectorText(text);
            }
            // Legacy/Direct checks (fallback)
            const url = (prop === null || prop === void 0 ? void 0 : prop.inspectorUrl) || (prop === null || prop === void 0 ? void 0 : prop.inspectorText); // fallback if just a url string
            if (url && typeof url === "string" && /^https?:\/\//.test(url) && url !== _lastInspectorUrl) {
                _lastInspectorUrl = url;
                addXyzLayer(url, prop === null || prop === void 0 ? void 0 : prop.inspectorTitle);
            }
        }
        catch (e) {
            // ignore
...
function loadInfoUrl(url) {
    ...
    try {
        ...
        try {
            postToUI({ action: 'loadInfoUrl', url: url });
        }
        catch (e) { }
    }
    catch (e) {
...
(function tryAutoFlyToFromUrl() {
    try {
            ...
            try {
                ...
                if (query) {
                    ...
                    if (typeof state.lat === 'number' && !isNaN(state.lat) && typeof state.lng === 'number' && !isNaN(state.lng)) {
                        applyPermalinkFromObject(state);
                        try {
                            sendLog('[tryAutoFlyToFromUrl] auto flyTo from viewport query:', query);
                        }
                        catch (e) { }
                    }
                    else {
                        try {
                            sendLog('[tryAutoFlyToFromUrl] no lat/lng in viewport query:', query);
                        }
                        catch (e) { }
                    }
                }
                    ...
                    try {
                        sendLog('[tryAutoFlyToFromUrl] viewport query unavailable');
                    }
                    catch (e) { }
                }
            }
                ...
                try {
                    sendError('[tryAutoFlyToFromUrl] error:', e);
                }
                catch (err) { }
            }
        };
        ...
        try {
            sendError('[tryAutoFlyToFromUrl] outer error:', e);
        }
        catch (err) { }
    }
})();
