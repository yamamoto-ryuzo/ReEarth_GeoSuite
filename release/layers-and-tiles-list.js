    ...
    const parseGroupPath = (groupStr) => {
        ...
        while (i < groupStr.length) {
            if (groupStr[i] === '/') {
                // count slashes
                let j = i;
                while (j < groupStr.length && groupStr[j] === '/')
                    j++;
        ...
        if (cur !== '')
            res.push({ seg: cur, exclusiveAfter: false });
        return res;
    };
    const buildTree = (arr) => {
    ...
    try {
        ...
        if (addMarkerFlag && typeof addTargetMarker === 'function' && !isNaN(lat) && !isNaN(lng)) {
            ...
            if (!layerId) {
                try {
                    try {
                        sendLog('[flyToAndNotify] addTargetMarker returned null, attempting direct reearth.layers.add fallback');
                    }
                    catch (e) { }
                    if (reearth && reearth.layers && typeof reearth.layers.add === 'function') {
                        ...
                        try {
                            ...
                            if (newId) {
                                ...
                                layerId = newId;
                                try {
                                    sendLog('[flyToAndNotify] fallback reearth.layers.add succeeded', layerId);
                                }
                                catch (e) { }
                            }
                            else {
                                try {
                                    sendError('[flyToAndNotify] fallback reearth.layers.add returned falsy id');
                                }
                                catch (e) { }
                            }
                        }
                            ...
                            try {
                                sendError('[flyToAndNotify] fallback reearth.layers.add threw', e);
                            }
                            catch (_) { }
                        }
                    }
                        ...
                        try {
                            sendError('[flyToAndNotify] no reearth.layers.add available for fallback');
                        }
                        catch (e) { }
                    }
                }
                    ...
                    try {
                        sendError('[flyToAndNotify] fallback error', e);
                    }
                    catch (_) { }
                }
            }
...
function processInspectorText(text) {
    ...
    lines.forEach(line => {
        ...
        if (lowerLine.startsWith('legend:')) {
            ...
            if (url)
                legends.push(encodeNonAscii(url));
    ...
    try {
        _inspectorLegendUrls = (legends && legends.length) ? legends.map(u => { try {
            return encodeNonAscii(u);
        }
        catch (e) {
            return u;
        } }) : [];
            ...
            setTimeout(function () {
                ...
                try {
                    if (legends && legends.length > 0)
                        postToUI({ action: 'updateLegends', urls: legends });
                }
                catch (e) {
                    try {
